...
\begin{itemize}
    ...
    \item \textbf{Protección de contraseñas}: el registro y el cambio de contraseña almacenan un hash bcrypt en la base de datos \texttt{Usuarios}. El cliente aplica un preprocesado adicional antes del envío; el detalle de este flujo híbrido se documenta en el capítulo de implementación.
...
La autenticación se basa en tokens JWT. Tras un inicio de sesión correcto, el cliente incluye el token en el encabezado \texttt{Authorization: Bearer ...} en las peticiones a endpoints protegidos. La API aplica control de acceso por rol mediante un filtro previo a la ejecución de cada endpoint sensible.
...
\begin{itemize}
    \item \textbf{\texttt{Repositorio\_Plantas}}: almacena las imágenes, su metadatos, las clases, las etiquetas dinámicas y la información asociada a experimentos y entrenamientos.
    \item \textbf{\texttt{Usuarios}}: almacena las credenciales y los roles de los usuarios del sistema.
\end{itemize}

Dentro de \texttt{Repositorio\_Plantas}, las colecciones más relevantes son:
...
Las credenciales y roles se aislaron en la base \texttt{Usuarios}, separada de \texttt{Repositorio\_Plantas}, para acotar permisos y copias de seguridad: la información de autenticación no se mezcla con el volumen creciente de documentos de imagen.
...
\begin{enumerate}
    ...
    \item \texttt{LogicaApp} envía una petición \texttt{POST} a \texttt{/iniciar\_sesion} con las credenciales procesadas por el cliente.
...
\begin{itemize}
    ...
    \item \texttt{backup\_mongo.sh}: copia de seguridad de MongoDB.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Otras importaciones}: datos aportados externamente (p. ej. vid, VRAIN) u otros conjuntos de Kaggle, siempre bajo \texttt{data/<fuente>/}.
\end{itemize}
