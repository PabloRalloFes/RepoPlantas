...
\begin{itemize}
    ...
    \item \texttt{backup\_mongo.sh}: copia de seguridad de las bases \texttt{Repositorio\_Plantas} y \texttt{Usuarios} (véase capítulo~9).
\end{itemize}
...
Los datos persistentes del sistema se reparten entre el volumen Docker de MongoDB (\texttt{mongo\_data}), las bases \texttt{Repositorio\_Plantas} y \texttt{Usuarios}, y los directorios montados \texttt{imagenes/}, \texttt{models/} y \texttt{experiments/}. Para la base de datos documental, el repositorio incluye el script \texttt{scripts/backup\_mongo.sh}, que genera un volcado (\texttt{mongodump}) de \textbf{ambas bases} en ficheros \texttt{.archive} con marca temporal bajo \texttt{backups/mongo/}.
