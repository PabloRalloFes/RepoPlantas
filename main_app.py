...
if __name__ == "__main__":
    ...
    def main(page: ft.Page):
        ...
        def safe_client_storage_set(key: str, value):
            try:
        ...
        def route_change(route):
            ...
            if page.route.startswith("/configuracion"):
                ...
                def guardar_url_api(e):
                    ...
                    storage_ok = safe_client_storage_set("url_api", logica_app.url_api)
                    mensaje = res["message"]
                    if not storage_ok:
                        mensaje += "\n\nAviso: no se pudo guardar en almacenamiento local del navegador."
                    page.open(ft.AlertDialog(
                        modal=True,
                ...
                def guardar_url_bbdd(e):
                    ...
                    storage_ok = safe_client_storage_set("url_bbdd", logica_app.url_bbdd)
            ...
            if page.route == ("/main_usuario"):
                ...
                def open_file_picker(allow_multiple=False):
                    # Ensure the FilePicker is attached to the page before calling pick_files()
                    if getattr(file_picker, "page", None) is None:
                        try:
                            page.overlay.append(file_picker)
                            page.update()
                        except Exception:
                    ...
                    # wait briefly for the control to be attached
                    for _ in range(3):
                        if getattr(file_picker, "page", None) is not None:
                            break
                        time.sleep(0.05)

                    try:
                        file_picker.pick_files(allow_multiple=allow_multiple)
                    except Exception:
                        # Last-resort attempt (will raise if still not attached)
                        try:
                            file_picker.pick_files(allow_multiple=allow_multiple)
                        except Exception:
                            pass
                                                        ...
                                                        content=ft.Column(
                                                            ...
                                                            controls=[
                                                                ...
                                                                ft.ElevatedButton(
                                                                    ...
                                                                    bgcolor=ft.Colors.GREEN,
                                                                    width=280,
                                                                    on_click=lambda _: open_file_picker(allow_multiple=False)
                                                                ),
                                                            ]
                                                        )
                                                    ),
            ...
            if page.route == "/main_usuario/subida_masiva":
                    ...
                    def open_zip_picker(allowed_extensions=None, dialog_title=None):
                        # Ensure the FilePicker is attached before calling pick_files()
                        if getattr(zip_picker, "page", None) is None:
                            try:
                                page.overlay.append(zip_picker)
                                page.update()
                            except Exception:
                        ...
                        for _ in range(3):
                            if getattr(zip_picker, "page", None) is not None:
                                break
                            time.sleep(0.05)

                        try:
                            zip_picker.pick_files(allowed_extensions=allowed_extensions, dialog_title=dialog_title)
                        except Exception:
                            try:
                                zip_picker.pick_files(allowed_extensions=allowed_extensions, dialog_title=dialog_title)
                            except Exception:
                                                                        ...
                                                                        ft.Column(
                                                                            ...
                                                                            controls=[
                                                                                ft.ElevatedButton(
                                                                                    ...
                                                                                    on_click=lambda _: open_zip_picker(
